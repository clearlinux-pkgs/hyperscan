...
Source0  : https://github.com/intel/hyperscan/archive/v5.0.0.tar.gz
Summary  : Intel(R) Hyperscan Library
Group    : Development/Tools
License  : BSD-3-Clause Intel
Requires: hyperscan-lib
Requires: hyperscan-license
...
%prep
%setup -q -n hyperscan-5.0.0
pushd ..
cp -a hyperscan-5.0.0 buildavx2
popd
pushd ..
cp -a hyperscan-5.0.0 buildavx512
popd
...
export LANG=C
export SOURCE_DATE_EPOCH=1531146153
mkdir clr-build
pushd clr-build
...
export CFLAGS="$CFLAGS -O3 -march=skylake-avx512 -mprefer-vector-width=512 "
export FCFLAGS="$CFLAGS -O3 -march=skylake-avx512 -mprefer-vector-width=512 "
export FFLAGS="$CFLAGS -O3 -march=skylake-avx512 -mprefer-vector-width=512 "
export CXXFLAGS="$CXXFLAGS -O3 -march=skylake-avx512 -mprefer-vector-width=512 "
...
cmake .. -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX=/usr -DBUILD_SHARED_LIBS:BOOL=ON -DLIB_INSTALL_DIR:PATH=/usr/lib64/haswell/avx512_1 -DCMAKE_AR=/usr/bin/gcc-ar -DCMAKE_RANLIB=/usr/bin/gcc-ranlib
...
export SOURCE_DATE_EPOCH=1531146153
rm -rf %{buildroot}
mkdir -p %{buildroot}/usr/share/doc/hyperscan
cp LICENSE %{buildroot}/usr/share/doc/hyperscan/LICENSE
cp COPYING %{buildroot}/usr/share/doc/hyperscan/COPYING
...
/usr/lib64/haswell/avx512_1/libhs.so.5.0.0
/usr/lib64/haswell/libhs.so.5
/usr/lib64/haswell/libhs.so.5.0.0
/usr/lib64/haswell/libhs_runtime.so.5
/usr/lib64/haswell/libhs_runtime.so.5.0.0
/usr/lib64/libhs.so.5
/usr/lib64/libhs.so.5.0.0
/usr/lib64/libhs_runtime.so.5
/usr/lib64/libhs_runtime.so.5.0.0

%files license
%defattr(-,root,root,-)
/usr/share/doc/hyperscan/COPYING
/usr/share/doc/hyperscan/LICENSE
